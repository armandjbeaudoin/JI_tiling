...
The user's projection embeds **A₄ ⊂ A₅** by treating the harmonic "1" as the
trivial generator: in 5D, the exponent of "1" is identically zero. Concretely,
in `eiko_6` the first column is dropped to give `eiko_5 = eiko_6[:,1:]`, the
5D embedding used for the projection.
...
| Hits per variant                              | 10 = 1 + 6 + 3 | origin + roots + weights |
| Diamond points covered by union of variants   | 31 / 31 | I₂⁵ symmetry of diamond + variants |
| Total distinct points in union                | 111 | 31 in diamond + 80 outside |

---

## 10. Follow-Ups and Open Directions

The Coxeter-orbit construction (option B in
`eikosany_diamond_coxeter.ipynb`) generalises in two natural directions.

### 10.1 The inner-ring overlay — the *other* C₅-orbit pair

The 10 outer-ring divisors at radius τ are one C₅-orbit pair
(Λ₂-adjacent ⊔ Λ₃-complement-adjacent). The 10 inner-ring Eikosany points at
radius 1/τ are the **other** C₅-orbit pair, generated by switching to
*non-adjacent* seeds:

| Ring | Λ₂ seed | Λ₃ seed | Orbit (musical) |
|---|---|---|---|
| outer (radius τ)   | ε₀ + ε₁    | ε₀ + ε₁ + ε₂  | 15, 35, 63, 99, 33  /  105, 315, 693, 297, 165 |
| **inner (radius 1/τ)** | **ε₀ + ε₂** | **ε₀ + ε₁ + ε₃** | (Λ₂ non-adjacent) / (Λ₃ non-adjacent complement) |

Translating the Eikosany by each of the 10 inner-ring points would give a
second family of 10 overlay variants. By the same {I₂⁵, A₄} Coxeter-pair
argument they form a single I₂⁵ orbit at the Galois-conjugate radius. Open
questions:

- How many of the 20 Eikosany points in each inner-ring variant land on the
  Hexadic Diamond? The answer is again given by orbit-difference combinatorics
  (P − P′ where P, P′ ∈ Λ₂ ⊔ Λ₃, with P now non-adjacent), and the count
  pattern may differ from the outer-ring 10 = 1 + 6 + 3.
- Do the inner-ring variants and the outer-ring variants together saturate
  the diamond, or do they cover overlapping subsets? (Together that's 20
  variants; individual coverage and joint coverage are both worth checking.)

This is a pure code change in the existing notebook — only the seeds in the
`coxeter_orbit()` call need to be swapped.

### 10.2 Other quadratic Coxeter pairs
...
The combinatorial bookkeeping (how many Eikosany points hit the diamond per
variant, how many variants are needed to cover the whole diamond, what
Galois-conjugate scale relates the inner and outer rings) will be specific
to each case but should follow from the same orbit-difference combinatorics
worked out in §7 above.
...
C(6, 3) into Λ₂ ⊔ Λ₃, projected through the {I₂⁵, A₄} Coxeter pair; the 10
variants are the I₂⁵ orbit of any chosen Λ₂- or Λ₃-translate, and each
variant's 10 shared points decompose as 1 origin + 6 roots + 3 weights —
purely from orbit-difference combinatorics in A₄.**

**The 10 variants saturate the Hexadic Diamond exactly: every one of its 31
points is reached, with multiplicities 10 (origin), 3 (each of 10 weights),
2 (each of 10 d=1 roots), 4 (each of 10 d=2 roots), summing to
10 + 30 + 20 + 40 = 100 hits = 10 variants × 10 hits.**
